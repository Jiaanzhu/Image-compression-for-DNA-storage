...
class JpegDNA():
    ...
    def __init__(self, alpha, sampler="4:2:2", verbosity=False, verbose=0):
        self.alpha = alpha
    ...
    def encode(self, inp, *args):
        ...
        if len(inp.shape) == 3:
            ...
            raise ValueError
        if image_type == "RGB":
            self.codec = JPEGDNARGB(self.alpha, formatting=True, channel_sampler=self.sampler, verbosity=self.verbosity, verbose=self.verbose)
        elif image_type == "gray":
            self.codec = JPEGDNAGray(self.alpha, formatting=True, verbosity=self.verbosity, verbose=self.verbose)
    ...
    def decode(self, code):
        ...
        if image_type == "RGB":
            ...
            self.codec = JPEGDNARGB(alpha, formatting=False, channel_sampler=sampler, verbosity=self.verbosity, verbose=self.verbose)
        elif image_type == "gray":
            self.codec = JPEGDNAGray(alpha, formatting=False, verbosity=self.verbosity, verbose=self.verbose)
        else:
            raise ValueError
