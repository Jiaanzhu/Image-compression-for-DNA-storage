...
class JPEGDNAGray(AbstractCoder):
    ...
    GAMMAS_CHROMA = np.array([[17, 18, 24, 47, 99, 99, 99, 99],
                              ...
                              [99, 99, 99, 99, 99, 99, 99, 99],
                              [99, 99, 99, 99, 99, 99, 99, 99]])
    def __init__(self, alpha, formatting=False, channel_type="luma", verbose=False, verbosity=0):
        ...
        self.remain = ""
        if self.formatting:
            self.formatter = JpegDNAFormatter(self.alpha, "gray", None, primer="illumina", oligo_length=200, debug=False)
        else:
            self.formatter = None
    ...
    def set_frequencies_from_img(self, inp):
    ...
    def full_decode(self, code, *args):
        ...
        if args[0] in ["default", "from_file"]:
            # Cheating with the position of the codeword for #EOB (the codeword should not be too short)
            dummy_freq_ac_idx = np.argpartition(self.freq_ac, -EOB_SHIFT_IDX)[-EOB_SHIFT_IDX:]
