...
class DataFormatter(AbstractFormatter):
    ...
    DEFAULT_OFFSET_CODE_LENGTH = 8
    def __init__(self, header, oligo_length=200, debug=False):
    ...
    def format(self, inp):
        ...
        :rtype: list(str)
        """
        data_payload_length = self.oligo_length - self.DEFAULT_OFFSET_CODE_LENGTH
        if self.debug:
            oligos = [self.data_info_header +
                      "\033[36m" +
                      self.codebook[self.DEFAULT_OFFSET_CODE_LENGTH - 2][i//data_payload_length] +
                      "\033[0m" +
        ...
        if self.debug:
            oligos[-1] = (oligos[-1] +
                          "\033[30;47m" +
                          generate_random_strand(data_payload_length-compute_length(oligos[-1]) + self.header_len + self.DEFAULT_OFFSET_CODE_LENGTH,
                                                 before,
                                                 "A") +
                          "\033[0;37;40m")
        else:
            oligos[-1] = (oligos[-1] +
                          generate_random_strand(data_payload_length-len(oligos[-1]) + self.header_len + self.DEFAULT_OFFSET_CODE_LENGTH,
                                                 before,
                                                 "A"))
        return oligos
    ...
    def deformat(self, oligos):
        ...
        max_offset = 0
        for oligo in oligos:
            if self.codebook[self.DEFAULT_OFFSET_CODE_LENGTH - 2].index(oligo[:self.DEFAULT_OFFSET_CODE_LENGTH]) > max_offset:
                max_offset = self.codebook[self.DEFAULT_OFFSET_CODE_LENGTH - 2].index(oligo[:self.DEFAULT_OFFSET_CODE_LENGTH])
        data_strands = [None] * (max_offset + 1)
        for oligo in oligos:
            data_strands[self.codebook[self.DEFAULT_OFFSET_CODE_LENGTH - 2].index(oligo[:self.DEFAULT_OFFSET_CODE_LENGTH])] = oligo[self.DEFAULT_OFFSET_CODE_LENGTH:]
