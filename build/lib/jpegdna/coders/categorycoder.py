...
class ACCategoryCoder(AbstractCoder):
    ...
    def decode(self, code):
        ...
        if category == 0:
            ...
            ad_bits = category+1
            #ad_bits = min(category+1, 9)
        self.ad_bits = ad_bits
        self.code_length = code_length
        if category < 160:
            if category in [7,8,9,17,18,19,27,28,29,37,38,39,47,48,49,57,58,59,67,68,69,77,78,79,87,88,89,97,98,99,107,
                            108,109,117,118,119,127,128,129,137,138,139,147,148,149,157,158,159]:
                raise NonDecodableCategory()
            else:
